...
<body class="min-h-screen flex flex-col items-center justify-center bg-black text-[#00FF00]">
    <h1 class="text-2xl font-bold mb-8">中控台</h1>
    <div id="adminPanel" class="flex flex-col items-center">
        ...
        <button id="deactivateAllScriptsBtn" class="px-6 py-3 rounded bg-blue-600 text-white font-bold text-lg hover:bg-blue-400 transition mb-4">停用所有腳本</button>
    ...
    document.getElementById('deactivateAllScriptsBtn').onclick = function() {
        if (!confirm('確定要將所有腳本設為停用？')) return;
        fetch('/admin/deactivate-all-scripts', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            },
